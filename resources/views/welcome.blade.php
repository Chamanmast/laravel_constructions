<x-front-layout>
    ...
    @php
        ...
        $blogs = App\Models\Blog::select('post_title', 'post_slug', 'post_image', 'short_descp')
            ->active(0)
            ->front(1)
            ->get();
        $module = App\Models\Module::select('heading', 'image', 'text')->find(4);
        $vision = App\Models\Module::select('heading', 'image', 'text')->find(7);
        $categories = App\Models\Category::with('services:category_id,name,slug,image,small_text')
            ->select('id', 'name')
            ->whereFront(1)
            ->whereType(0)
            ->get();
        $brands = App\Models\Brand::select('name', 'image')
            ->active(0)
            ->whereIn('id', [1,2,9,5,4,3])
            ->get();
