                                ...
                                <tbody>
                                    @foreach ($blog as $post)
                                        <tr class="blog-{{ $post->id }}">
                                            ...
                                            <td>{{ $post->created_at->format('l d M Y h:i A') }}</td>
                                            <td class="text-center">
                                                <button type="button"
                                                    onClick="statusFunction( {{ $post->id }} ,'Blog')"
                                                    class="shadow-none badge badge-light-{{ $post->status == 1 ? 'danger' : 'success' }} warning changestatus{{ $post->id }}  bs-tooltip"
                                                    data-toggle="tooltip" data-placement="top" title="Status"
                                                    data-original-title="Status">{{ $post->status == 1 ? 'Deactive' : 'Active' }}</button>
