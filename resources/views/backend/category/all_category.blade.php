    ...
    <div class="page-content">
        ...
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h6 class="card-title fw-bold">All Category</h6>
                        ...
                        <div class="table-responsive">
                                    ...
                                    @foreach ($category as $cat)
                                        <tr class="category-{{ $cat->id }}">
                                            ...
                                            <td class="text-center">
                                                <button type="button"
                                                    onClick="statusFunction({{ $cat->id }},'Category')"
                                                    class="shadow-none badge badge-light-{{ $cat->status == 1 ? 'danger' : 'success' }} warning changestatus{{ $cat->id }}  bs-tooltip"
                                            ...
                                            <td class="text-center">
                                                <div class="action-btns">
                                                    ...
                                                    <a href="javascript:void(0)"
                                                        onClick="deleteFunction({{ $cat->id }},'Category')"
                            ...
                            @if ($category->count() != 0)
                                <div class="ms-3">
                                    <button id="deleteall" onClick="deleteAllFunction('Category')"
    ...
    @if ($category->count() != 0)
        <script type="text/javascript">
            function deleteAllFunction(table) {
                ...
                var checkboxes = document.catrySelectorAll('.mixed_child');
            ...
            function deleteFunction(id, table) {
                ...
                document.catrySelector('.delete' + id).addEventListener('click', function() {
                    Swal.fire({
                        title: 'Are you sure?',
                        ...
                        if (result.isConfirmed) {
                            ...
                            setTimeout(function() {
                                var elems = document.catrySelector('.category-' + id);
                                elems.remove();
                                var crf = '{{ csrf_token() }}';
                                $.cat("{{ route('category.delete') }}", {
                                    _token: crf,
