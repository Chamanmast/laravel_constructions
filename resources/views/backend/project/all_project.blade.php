                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                                ...
                                <tbody>
                                    @foreach ($projects as $item)
                                        <tr class="project-{{ $item->id }}">
                                            ...
                                            <td> <span
                                                    class="badge badge-light-{{ $item->popular == 1 ? 'secondary' : 'success' }}">{{ $item->popular == 1 ? 'Popular' : 'Normal' }}</span>
                                            </td>
                                            <td>{{ $item->service->name }}</td>
                                            <td>{{ $item->name }}</td>
                                            <td class="text-center">
                                                <button type="button" onClick="statusFunction( {{$item->id}} ,'project')"
                                                    class="shadow-none badge badge-light-{{ $item->status == 1 ? 'danger' : 'success' }} warning changestatus{{ $item->id }}  bs-tooltip"
                                                    data-toggle="tooltip" data-placement="top" title="Status"
                                                    data-original-title="Status">{{ $item->status == 1 ? 'Deactive' : 'Active' }}</button>
                                            ...
                                            <td class="text-center">
                                                <div class="action-btns">
                                                    ...
                                                    <a href="javascript:void(0)"
                                                        onClick="deleteFunction({{ $item->id }},'project')"
                            ...
                            @if ($projects->count() != 0)
                                <div class="ms-3">
                                    <button id="deleteall" onClick="deleteAllFunction('project')"
                ...
                if (checkedValues.length === 0) {
                    ...
                    checkboxes.forEach(function(checkbox) {
                        ...
                        if (checkbox.checked) {
                            // Add the value to the array
                            checkedValues.push(checkbox.value);
                            var elems = document.querySelector('.project-' + checkbox.value);
                            elems.remove();
                        }
                    });
                    // console.log("Checked Checkbox Values: ", checkedValues);
                    var crf = '{{ csrf_token() }}';
                    $.post("{{ route('project.delete') }}", {
                        _token: crf,
                        id: checkedValues,
                        table: table,
                    }, function(data) {
                ...
                document.querySelector('.delete' + id).addEventListener('click', function() {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            setTimeout(function() {
                                var elems = document.querySelector('.project-' + id);
                                elems.remove();
                                var crf = '{{ csrf_token() }}';
                                $.post("{{ route('project.delete') }}", {
                                    _token: crf,
